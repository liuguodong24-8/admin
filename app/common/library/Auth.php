...
class Auth extends \ba\Auth
{
     ...
     * @return bool
     */
    public function register(string $username, string $password, string $mobile = '', string $email = '', int $group = 1, array $extend = [], string $default_salt = ''): bool
    {
        $validate = Validate::rule([
//            'mobile'   => 'mobile|unique:user',
//            'email'    => 'email|unique:user',
            'username' => 'regex:^[a-zA-Z][a-zA-Z0-9_]{2,20}$|unique:user',
            'password' => 'regex:^(?!.*[&<>"\'\n\r]).{6,32}$',
        ]);
        $params   = [
            'username' => $username,
...
//            'email'    => $email,
        ...
        addLog(var_export($params,true),'_sp_data_auth_register.log');
        addLog($validate->check($params),'_sp_data_auth_register.log');
        if (!$validate->check($params)) {
            $this->setError('Registration parameter error');
            return false;
        }
        addLog('下一步','_sp_data_auth_register.log');
        $ip   = request()->ip();
        $time = time();
        $salt = $default_salt == '' ? Random::build('alnum', 16) : $default_salt;
        $data = [
        ...
        addLog('到 事务面前了','_sp_data_auth_register.log');
        addLog(var_export($data,true),'_sp_data_auth_register.log');
        Db::startTrans();
        try {
    ...
    public function login(string $username, string $password, bool $keep): bool
    {
        ...
        $validate    = Validate::rule([
            'mobile'   => 'mobile',
            'email'    => 'email',
            'username' => 'regex:^[a-zA-Z][a-zA-Z0-9_]{2,20}$',
        ]);
